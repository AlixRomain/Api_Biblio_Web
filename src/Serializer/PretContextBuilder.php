...
final class PretContextBuilder implements SerializerContextBuilderInterface
{
    ...
    public function createFromRequest(Request $request, bool $normalization, ?array $extractedAttributes = null): array
    {
        ...
        if ($resourceClass === Pret::class && isset($context['groups']) && $this->authorizationChecker->isGranted('ROLE_ADMIN') && false === $normalization) {
            if($request->getMethod()=="PUT"){
                //Alors je r'ajoute le groups put_admin au tableau de groups par défaut.
                //On pourrais aussi imaginer que j'en enlève..
                $context['groups'][] = 'put_admin';
            }else if(($request->getMethod()=="POST") && $this->authorizationChecker->isGranted('ROLE_ADHERENT')){
